            ...
            if (turnNumber == 2) {
                ...
                if (answer == "no") {
                    ...
                    for (int i = 0; i < player2.getInplayCards().size(); i++) {
                        if ((player2.getInplayCards()[i].getTypeofCard() == "Creature")) {
                            for (int j = 0; j < creatureCards.size(); j++) {
                                if(player2.getInplayCards()[i].getName() == creatureCards[j].getName()){
 ...
 * IN THIS FUNCTION. BUT WE HAD TROUBLE IN COMBINING THE GAME CLASS
 * WITH THE ATTACK FUNCTION WE WROTE. BECAUSE THE PROGRESS OF THE TWO
 * STRUCTURES WAS DIFFERENT, WE DID NOT HAVE ENOUGH TIME TO CONNECT. RESPECTS.
 */
...
//            DC -> atak<defans , ( AC->yasiyor DC-> oldu , Ayrica playerin cani gitti )
